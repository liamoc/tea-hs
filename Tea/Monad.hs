...
eventStateModify f = modify $ \ts@(TS {_eventState = es}) -> ts {_eventState = f es}
...
getEventQuery :: EventQuery -> Tea Bool
...
queryKeyCode code  = (! code)     <$> keyCodes    <$> _eventState <$> get
queryKeyDown       = (> 0)        <$> keysDown    <$> _eventState <$> get
...
(?) :: EventQuery -> Tea z -> Tea()
q ? m = do a <- getEventQuery q
           if a 
              then m >> return ()
              else return ()

is :: EventQuery -> Tea Bool
is = getEventQuery

setFrameRate :: Int ->  Tea ()
setFrameRate n = modify $ \ts -> ts { _fpsCap = 1000 `div` n }
